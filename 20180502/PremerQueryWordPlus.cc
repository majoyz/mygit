...
ostream &print(ostream & os,const QueryResult &qr){//打印QueryResult查询结果
	...
	for(auto num:*qr._lines)
		os << "(line " << num + 1 << ") "
			<< *(qr._file->begin() + num) << endl;//用迭代器的方法打印
	return os;
}
...
public:
	...
	{return q->rep();}
private:
	Query(shared_ptr<Query_base> query)//下面的隐式转换的实现，再由这个q调用eval
	:q(query)
	{}
...
inline Query operator&(const Query &lhs,const Query &rhs)//与操作的定义，返回一个Query_base指针，隐式转换成Query对象
{
...
QueryResult NotQuery::eval(const TextQuery & text) const
{
	...
	auto beg = result.begin(),end = result.end();//正常查询的结果的开始和结尾迭代器
