...
WebPage::WebPage(string & doc/*,Simhasher & simhasher,SplitTool & jieba,Configuration & config,WordSegmentation & jieba*/)
//:_doc(doc)
{
...
/*	boost::regex getDocid("(?<=<docid>).*?(?=</docid>)");
    ...
    vector<pair<string ,double> > res;
	cout << "problem 0" << endl;
    simhasher1.extract(docContent, res, topN);
	cout << "problem 1" << endl;
    simhasher1.make(docContent, topN, u64);
	_simhash = u64;
    cout << "关键词序列是: " << res << endl;
    cout<< "simhash值是: " << u64<<endl;
