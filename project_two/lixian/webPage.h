...
class WebPage{//网页类
	public:
		WebPage(string & doc/*,Simhasher & simhasher,SplitTool &,Configuration & config,WordSegmentation & jieba*/);//构造函数
/*		int getDocId();//获取文档的docid
		string getDoc();//获取文档
		map<string,int> & getWordMap();//获取文档的词频统计map
		uint64_t getSimhash();
	private:
		void processDoc(const string & doc,Configuration & configi,WordSegmentation &);//对格式化文档进行处理
